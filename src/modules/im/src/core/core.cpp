...
void Core::registerCallbacks(Tox* tox) {
    ...
    connect(tox, &lib::messenger::Messenger::connected, [this]() { emit connected(); });
}
...
void Core::checkLastOnline(QString friendId) {
    ...
    //    emit friendLastSeenChanged(receiver, QDateTime::fromTime_t(lastOnline));
    //  }
}
...
void Core::setUIStarted() { tox->setUIStarted(); }
