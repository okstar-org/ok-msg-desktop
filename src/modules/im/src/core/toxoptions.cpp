 ...
 *        proxy address data. This is needed to ensure both have equal lifetime
 * and are correctly deleted.
 */

ToxOptions::ToxOptions(Tox_Options *options, const QByteArray &proxyAddrData)
    : options(options), proxyAddrData(proxyAddrData) {}
 ...
 */
std::unique_ptr<ToxOptions>
ToxOptions::makeToxOptions(const QByteArray &savedata, const ICoreSettings *s) {
...
//  }
  ...
  //
      auto toxOptions = std::unique_ptr<ToxOptions>(new ToxOptions(tox_opts,
      proxyAddr.toUtf8()));
  //    // register log first, to get messages as early as possible
  //    tox_options_set_log_callback(*toxOptions, ToxLogger::onLogMessage);
  //
  //    // savedata
  //    tox_options_set_savedata_type(*toxOptions, savedata.isNull() ?
  //    TOX_SAVEDATA_TYPE_NONE
  //                                                                 :
  //                                                                 TOX_SAVEDATA_TYPE_TOX_SAVE);
  //    tox_options_set_savedata_data(*toxOptions, reinterpret_cast<const
  //    uint8_t*>(savedata.data()),
  //                                  savedata.size());
  //
  //    if(s == nullptr) {
  ...
  //    }
  //
  //    // IPv6 needed for LAN discovery, but can crash some weird routers. On
  //    by default, can be
  //    // disabled in options.
  //    const bool enableIPv6 = s->getEnableIPv6();
  //    bool forceTCP = s->getForceTCP();
  //    // LAN requiring UDP is a toxcore limitation, ideally wouldn't be
  //    related const bool enableLanDiscovery = s->getEnableLanDiscovery() &&
  //    !forceTCP; ICoreSettings::ProxyType proxyType = s->getProxyType();
  //    quint16 proxyPort = s->getProxyPort();
  //
  //    if (!enableLanDiscovery) {
  //        qWarning() << "Core starting without LAN discovery. Peers can only
  //        be found through DHT.";
  //    }
  //    if (enableIPv6) {
  //        qDebug() << "Core starting with IPv6 enabled";
  //    } else if (enableLanDiscovery) {
  //        qWarning() << "Core starting with IPv6 disabled. LAN discovery may
  //        not work properly.";
  //    }
  //
  //    // No proxy by default
  ...
  //        if (static_cast<uint32_t>(proxyAddr.length()) >
  ...
  //                tox_options_set_proxy_type(*toxOptions,
  ...
  //            }
  //
  //            tox_options_set_proxy_host(*toxOptions,
  //            toxOptions->getProxyAddrData());
  //            tox_options_set_proxy_port(*toxOptions, proxyPort);
  //
  //            if (!forceTCP) {
  //                qDebug() << "Proxy and UDP enabled, this is a security risk,
  //                forcing TCP only"; forceTCP = true;
  //            }
  //        }
  //    }
  //
  ...
  //    tox_options_set_local_discovery_enabled(*toxOptions,
  //    enableLanDiscovery); tox_options_set_start_port(*toxOptions, 0);
