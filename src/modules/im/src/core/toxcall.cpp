      ...
      sink(audio.makeSink()), peerId{peerId}
{
  ...
  if (sink) {
    audioSinkInvalid = sink->connectTo_invalidated(
...
void ToxFriendCall::onAudioSinkInvalidated() {
  ...
  if (newSink) {
    audioSinkInvalid = newSink->connectTo_invalidated(
        this, [this]() { this->onAudioSinkInvalidated(); });
  }
...
void ToxFriendCall::playAudioBuffer(const int16_t *data, int samples,
                                    unsigned channels, int sampleRate) const {
  if (sink) {
...
ToxGroupCall::ToxGroupCall(const Group &group, CoreAV &av, IAudioControl &audio)
  ...
  connect(
      ...
      [this](const int16_t *pcm, size_t samples, uint8_t chans, uint32_t rate) {
        ...
        this->av->sendGroupCallAudio(this->group.getId(), pcm, samples, chans,
                                     rate);
      });
