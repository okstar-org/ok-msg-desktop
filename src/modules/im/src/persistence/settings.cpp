...
Settings::Settings()
    : loaded(false), useCustomDhtList{false}, makeToxPortable{false} {
...
void Settings::loadGlobal() {
  ...
  s.beginGroup("General");
  {
    ...
    autoAcceptMaxSize = static_cast<size_t>(
        s.value("autoAcceptMaxSize", 20 << 20 /*20 MB*/).toLongLong());
    stylePreference =
        static_cast<StyleType>(s.value("stylePreference", 1).toInt());
  }
  s.endGroup();
  ...
  s.beginGroup("GUI");
  {
    ...
    secondColumnHandlePosFromRight =
        s.value("secondColumnHandlePosFromRight", 50).toInt();
    ...
    showGroupJoinLeaveMessages =
        s.value("showGroupJoinLeaveMessages", false).toBool();
  ...
  {
    chatMessageFont =
        s.value("chatMessageFont", Style::getFont(Style::Big)).value<QFont>();
  }
  s.endGroup();
  // 隐私
  s.beginGroup("State");
  {
    ...
    dialogSplitterState =
        s.value("dialogSplitterState", QByteArray()).toByteArray();
    dialogSettingsGeometry =
        s.value("dialogSettingsGeometry", QByteArray()).toByteArray();
  }
  s.endGroup();
...
bool Settings::isToxPortable() {
  QString localSettingsPath =
      qApp->applicationDirPath() + QDir::separator() + globalSettingsFile;
...
bool Settings::verifyProxySettings(const QCommandLineParser &parser) {
  ...
  if (parser.isSet("P")) {
    activeProxyType =
        proxySettingStrings[0] == SOCKS5 || proxySettingStrings[0] == HTTP;
  }
...
void Settings::saveGlobal() {
  ...
  s.beginGroup("GUI");
  {
    ...
    s.setValue("secondColumnHandlePosFromRight",
               secondColumnHandlePosFromRight);
    ...
    s.setValue("statusChangeNotificationEnabled",
               statusChangeNotificationEnabled);
...
void Settings::loadPersonal(QString profileName, const ToxEncrypt *passKey) {
  ...
  ps.beginGroup("Friends");
  {
    ...
    for (int i = 0; i < size; i++) {
      ...
      fp.autoAcceptCall = Settings::AutoAcceptCallFlags(
          QFlag(ps.value("autoAcceptCall", 0).toInt()));
  ...
  ps.beginGroup("GUI");
  {
    ...
    sortingMode = static_cast<FriendListSortingMode>(
        ps.value("friendSortingMethod",
                 static_cast<int>(FriendListSortingMode::Name))
            .toInt());
  }
  ps.endGroup();
  ...
  ps.beginGroup("Proxy");
  {
    proxyType = static_cast<ProxyType>(
        ps.value("proxyType", 0 /* ProxyType::None */).toInt());
...
void Settings::setStatusChangeNotificationEnabled(bool newValue) {
  ...
  if (newValue != statusChangeNotificationEnabled) {
    ...
    emit statusChangeNotificationEnabledChanged(
        statusChangeNotificationEnabled);
  }
}
...
void Settings::sync() {
  if (QThread::currentThread() != settingsThread) {
    QMetaObject::invokeMethod(&getInstance(), "sync",
...
Settings::fixInvalidProxyType(ICoreSettings::ProxyType proxyType) {
