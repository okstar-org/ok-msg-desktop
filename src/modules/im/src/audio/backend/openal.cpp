...
OpenAL::OpenAL() : audioThread{new QThread} {
  ...
  QObject::connect(audioThread, &QThread::finished, &captureTimer,
                   &QTimer::stop);
  QObject::connect(audioThread, &QThread::finished, audioThread,
                   &QThread::deleteLater);
...
std::unique_ptr<IAudioSink> OpenAL::makeSink() {
  ...
  sinks.insert(sink);
  qDebug() << "Audio source" << sid
           << "created. Sources active:" << sinks.size();

  return std::unique_ptr<IAudioSink>{sink};
}

/**
 * @brief Must be called by the destructor of AlSink to remove the internal
 * ressources. If no sinks are opened, the output is closed afterwards.
...
void OpenAL::destroySink(AlSink &sink) {
  ...
  if (alIsSource(sid)) {
    ...
    cleanupBuffers(sid);
    qDebug() << "Audio source" << sid
             << "deleted. Sources active:" << sinks.size();
  } else {
...
std::unique_ptr<IAudioSource> OpenAL::makeSource() {
  ...
  qDebug() << "Subscribed to audio input device [" << sources.size()
           << "subscriptions ]";
...
bool OpenAL::initInput(const QString &deviceName, uint32_t channels) {
  ...
  const int safetyFactor =
      2; // internal OpenAL ring buffer. must be larger than our inputBuffer
         // to avoid the ring from overwriting itself between captures.
  AUDIO_FRAME_SAMPLE_COUNT_TOTAL =
      AUDIO_FRAME_SAMPLE_COUNT_PER_CHANNEL * channels;
  const ALCsizei ringBufSize =
      AUDIO_FRAME_SAMPLE_COUNT_TOTAL * bytesPerSample * safetyFactor;

  const QByteArray qDevName = deviceName.toUtf8();
  const ALchar *tmpDevName =
      qDevName.isEmpty() ? nullptr : qDevName.constData();
  alInDev = alcCaptureOpenDevice(tmpDevName, AUDIO_SAMPLE_RATE, stereoFlag,
                                 ringBufSize);
...
void OpenAL::playMono16Sound(AlSink &sink, const IAudioSink::Sound &sound) {
  ...
  alBufferData(bufid, AL_FORMAT_MONO16, data.constData(), data.size(),
               AUDIO_SAMPLE_RATE);
...
void OpenAL::playAudioBuffer(uint sourceId, const int16_t *data, int samples,
  ...
  alBufferData(bufids[0],
               (channels == 1) ? AL_FORMAT_MONO16 : AL_FORMAT_STEREO16, data,
...
float OpenAL::getVolume() {
  ...
  for (quint32 i = 0; i < samples; i++) {
    float sample = static_cast<float>(inputBuffer[i]) /
                   std::numeric_limits<int16_t>::max();
...
void OpenAL::doInput() {
  ...
  // NOTE(sudden6): this loop probably doesn't scale too well with many sources
  for (auto source : sources) {
    emit source->frameAvailable(inputBuffer,
                                AUDIO_FRAME_SAMPLE_COUNT_PER_CHANNEL, channels,
                                AUDIO_SAMPLE_RATE);
  }
}
...
void OpenAL::captureSamples(ALCdevice *device, int16_t *buffer,
                            ALCsizei samples) {
...
QStringList OpenAL::outDeviceNames() {
  ...
  const ALchar *pDeviceList =
      (alcIsExtensionPresent(nullptr, "ALC_ENUMERATE_ALL_EXT") != AL_FALSE)
