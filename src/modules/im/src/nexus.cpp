...
//    }
  ...
  void Nexus::setSettings(Settings * settings) {
    if (this->settings) {
      QObject::disconnect(this, &Nexus::saveGlobal, this->settings,
                          &Settings::saveGlobal);
    }
    this->settings = settings;
    if (this->settings) {
      QObject::connect(this, &Nexus::saveGlobal, this->settings,
                       &Settings::saveGlobal);
    }
  }
...
//  }
  ...
  void Nexus::onWindowStateChanged(Qt::WindowStates state) {
    ...
    if (QApplication::activeWindow() != nullptr &&
        sender() == QApplication::activeWindow()) {
