...
IChatItem::Ptr dateMessageForItem(const ChatLogItem &item) {
  ...
  return ChatMessage::createChatInfoMessage(dateText, ChatMessage::INFO,
                                            QDateTime());
}
...
#endif
  encryptButton = createButton("encryptButton", this,
                               &GenericChatForm::onEncryptButtonClicked);

  sendButton =
      createButton("sendButton", this, &GenericChatForm::onSendTriggered);

  emoteButton =
      createButton("emoteButton", this, &GenericChatForm::onEmoteButtonClicked);

  fileButton =
      createButton("fileButton", this, &GenericChatForm::onAttachClicked);
  screenshotButton = createButton("screenshotButton", this,
...
void GenericChatForm::addSystemInfoMessage(const QString &message,
  ...
  insertChatMessage(
      ChatMessage::createChatInfoMessage(message, type, datetime));
}
...
void GenericChatForm::handleSearchResult(SearchResult result,
...
void GenericChatForm::renderMessage(ChatLogIdx idx) {
  qDebug()<<__func__ << "for contact:" << contactId->toString() << "message log index:"<<idx.get();
