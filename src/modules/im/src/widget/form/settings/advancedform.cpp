...
AdvancedForm::AdvancedForm()
  ...
  const bool udpEnabled =
      !s.getForceTCP() && (s.getProxyType() == Settings::ProxyType::ptNone);
  ...
  settings::Translator::registerHandler(
      std::bind(&AdvancedForm::retranslateUi, this), this);
}
...
void AdvancedForm::on_btnExportLog_clicked() {
  QString savefile = QFileDialog::getSaveFileName(
      Q_NULLPTR, tr("Save File"), QString{}, tr("Logs (*.log)"));
  ...
  auto &logManager = ok::lib::LogManager::Instance();
  auto &file = logManager.getFile();
  if (!file.exists()) {
    qDebug() << "No log file found!";
    return;
  }

  if (QFile::copy(file.fileName(), savefile))
...
void AdvancedForm::on_resetButton_clicked() {
  ...
  bool result = GUI::askQuestion(
      titile, tr("All settings will be reset to default. Are you sure?"),
      tr("Yes"), tr("No"));
...
void AdvancedForm::on_cbEnableUDP_stateChanged() {
  ...
  const bool enableLanDiscovery =
      Settings::getInstance().getEnableLanDiscovery();
...
void AdvancedForm::on_cbEnableLanDiscovery_stateChanged() {
  Settings::getInstance().setEnableLanDiscovery(
      bodyUI->cbEnableLanDiscovery->isChecked());
}
