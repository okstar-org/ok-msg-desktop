...
static QImage getRectShadow(const QSize &size, int shadowRadius, const QBrush shadowBrush, const QBrush &backBrush, qreal rectRadius, qreal deviceScale) {
...
void ShadowBackground::drawShadow()
{
    ...
    if (shadowPix.isNull()) {
        ...
        shadowPix = getRectShadow(QSize(im_w, im_w), _shadowRadius, _shadowColor, _centerColor, _borderRadius, factor);
    }
    ...
    painter.drawImage(QRectF(anchor.topLeft(), corner_size), shadowPix, QRectF(im_anchor.topLeft(), im_corner_size));
    painter.drawImage(QRectF(anchor.topRight(), corner_size), shadowPix, QRectF(im_anchor.topRight(), im_corner_size));
    painter.drawImage(QRectF(anchor.bottomRight(), corner_size), shadowPix, QRectF(im_anchor.bottomRight(), im_corner_size));
    painter.drawImage(QRectF(anchor.bottomLeft(), corner_size), shadowPix, QRectF(im_anchor.bottomLeft(), im_corner_size));
    ...
    painter.drawImage(QRectF(anchor.topLeft(), he_size), shadowPix, QRectF(im_anchor.topLeft(), QSizeF(content, img_clip)));
    painter.drawImage(QRectF(anchor.bottomLeft(), he_size), shadowPix, QRectF(im_anchor.bottomLeft(), QSizeF(content, img_clip)));
    anchor.adjust(-clip, clip, 0, 0);
    im_anchor.adjust(-img_clip, img_clip, 0, 0);
    painter.drawImage(QRectF(anchor.topLeft(), ve_size), shadowPix, QRectF(im_anchor.topLeft(), QSizeF(img_clip, content)));
    painter.drawImage(QRectF(anchor.topRight(), ve_size), shadowPix, QRectF(im_anchor.topRight(), QSizeF(img_clip, content)));
