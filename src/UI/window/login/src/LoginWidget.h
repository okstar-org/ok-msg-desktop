...
signals:
    ...
    void loginResult(ok::session::SignInInfo& info, ok::session::LoginResult& result);

private slots:
    void onTimeout();
    void doLogin();
    void onConnectResult(ok::session::SignInInfo info, ok::session::LoginResult result);
