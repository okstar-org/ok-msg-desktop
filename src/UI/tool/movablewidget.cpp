...
void MovableWidget::setBoundary(QRect newBoundary) {
  ...
  float changeX =
      newBoundary.width() / static_cast<float>(boundaryRect.width());
  float changeY =
      newBoundary.height() / static_cast<float>(boundaryRect.height());

  float percentageX = (x() - boundaryRect.x()) /
                      static_cast<float>(boundaryRect.width() - width());
  float percentageY = (y() - boundaryRect.y()) /
                      static_cast<float>(boundaryRect.height() - height());
      ...
      if (event->buttons() & Qt::LeftButton) {
        ...
        if (mode & (ResizeLeft | ResizeRight)) {
          if (mode & (ResizeUp | ResizeDown)) {
            ...
            if (!(mode & ResizeDown))
              lastPosition.setY(lastPosition.y() -
