...
inline IM* getIM() { return ok::session::AuthSession::Instance()->im(); }
...
void IMJingle::setMute(bool mute) { OkRTCManager::getInstance()->getRtc()->setMute(mute); }
...
IMJingleSession* IMJingle::cacheSessionInfo(Jingle::Session* session,
    ...
    auto ws = new IMJingleSession(getIM(), peer, sId, callType, session, fileHandlers, this);
...
void IMJingle::onCreatePeerConnection(const std::string& sId, const std::string& peerId, bool ok) {
 ...
 * @param image
 */
void IMJingle::onRender(const std::string& peerId, lib::ortc::RendererImage image) {
    if (peerId.empty()) {
...
bool IMJingle::sendCallToResource(const QString& friendId, const QString& sId, bool video) {
...
bool IMJingle::answer(const IMPeerId& peerId, const QString& callId, bool video) {
 ...
 * @param file
 */
void IMJingle::rejectFileRequest(const QString& friendId, const QString& sId) {
    cancelCall(IMContactId{friendId}, sId);
}

void IMJingle::acceptFileRequest(const QString& friendId, const File& file) {
...
void IMJingle::finishFileRequest(const QString& friendId, const QString& sId) {
    qDebug() << __func__ << "sId:" << (sId);
    auto* s = findSession(sId);
    if (!s) {
...
void IMJingle::finishFileTransfer(const QString& friendId, const QString& sId) {
    qDebug() << __func__ << "sId:" << (sId);
    finishFileRequest(friendId, sId);
}

bool IMJingle::sendFile(const QString& friendId, const File& file) {
...
bool IMJingle::sendFileToResource(const JID& jid, const File& file) {
