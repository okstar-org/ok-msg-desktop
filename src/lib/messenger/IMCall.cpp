...
void IMCall::connectJingle(IMJingle* _jingle) {
     ...
     * callHandlers
     */
    connect(_jingle, &IMJingle ::receiveCallRequest, this,
    ...
    connect(_jingle, &IMJingle::receiveCallRetract, this, [&](QString friendId, int state) {
        for (auto handler : callHandlers) {
    ...
    connect(_jingle, &IMJingle::receiveFriendHangup, this, [&](QString friendId, int state) {
        for (auto handler : callHandlers) {
    ...
    connect(_jingle, &IMJingle::receiveFriendVideoFrame, this,
...
void IMCall::callRetract(const IMContactId& f, const QString& sId) { jingle->cancelCall(f, sId); }

void IMCall::callReject(const IMPeerId& f, const QString& sId) { jingle->rejectCall(f, sId); }

void IMCall::setMute(bool mute) { jingle->setMute(mute); }

void IMCall::setRemoteMute(bool mute) { jingle->setRemoteMute(mute); }
