...
class Pool : public std::enable_shared_from_this<Pool<ValueT, CreatorT>> {
    struct Entry {
        ...
        bool operator<(const Entry& other) const { return refcnt < other.refcnt; }
    };
...
public:
    ...
    std::shared_ptr<ValueT> get() {
        ...
        return std::shared_ptr<ValueT>(
                entries_[i].value.get(),
                [i, self = this->shared_from_this()](auto* ptr) { self->dec_ref(i); });
    }
...
public:
    explicit ThreadsImpl(size_t i) {
        ...
        media_ = create("ortc-media" + suffix);
        worker_ = create("ortc-work" + suffix);
        network_ = create_network("ortc-net" + suffix);
        //    worker_ = create_network("ortc-work" + suffix);
        // network_->DisallowAllInvokes();
        // worker_->DisallowAllInvokes();
        // worker_->AllowInvokesToThread(network_.get());
    }

    rtc::Thread* getNetworkThread() override { return network_.get(); }
    rtc::Thread* getMediaThread() override { return media_.get(); }
    rtc::Thread* getWorkerThread() override { return worker_.get(); }
    //  rtc::scoped_refptr<webrtc::SharedModuleThread> getSharedModuleThread() override {
    // This function must be called from a single thread because of SharedModuleThread
    // implementation So we don't care about making it thread safe
...
public:
    std::unique_ptr<Threads> operator()(size_t i) { return std::make_unique<ThreadsImpl>(i); }
};
...
void Threads::setPoolSize(size_t size) { get_pool().set_pool_size(size); }
std::shared_ptr<Threads> Threads::getThreads() { return get_pool().get(); }

namespace StaticThreads {

rtc::Thread* getNetworkThread() { return getThreads()->getNetworkThread(); }

rtc::Thread* getMediaThread() { return getThreads()->getMediaThread(); }

rtc::Thread* getWorkerThread() { return getThreads()->getWorkerThread(); }
