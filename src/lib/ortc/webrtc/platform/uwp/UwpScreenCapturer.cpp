...
UwpScreenCapturer::UwpScreenCapturer(
        ...
        : _sink(sink), item_(item) {}

UwpScreenCapturer::~UwpScreenCapturer() { destroy(); }
...
void UwpScreenCapturer::create() {
    ...
    if (hr == DXGI_ERROR_UNSUPPORTED) {
        ...
        hr = D3D11CreateDevice(
                ...
                /*software_rasterizer=*/nullptr, D3D11_CREATE_DEVICE_BGRA_SUPPORT,
                /*feature_levels=*/nullptr, /*feature_levels_size=*/0, D3D11_SDK_VERSION,
                d3d11_device_.put(), /*feature_level=*/nullptr,
    ...
    direct3d_device_->QueryInterface(
            winrt::guid_of<winrt::Windows::Graphics::DirectX::Direct3D11::IDirect3DDevice>(),
            winrt::put_abi(directDevice));

    frame_pool_ = Direct3D11CaptureFramePool::CreateFreeThreaded(directDevice, kPixelFormat,
                                                                 kNumBuffers, previous_size_);
...
// void UwpScreenCapturer::OnFrameArrived(Direct3D11CaptureFramePool const& sender,
// winrt::Windows::Foundation::IInspectable const&) {
void UwpScreenCapturer::OnFrameArrived(DispatcherQueueTimer const& sender,
    ...
    // new size.
    if (previous_size_.Height != new_size.Height || previous_size_.Width != new_size.Width) {
...
void UwpScreenCapturer::OnClosed(GraphicsCaptureItem const& sender,
                                 winrt::Windows::Foundation::IInspectable const&) {
    winrt::slim_lock_guard const guard(lock_);
...
HRESULT UwpScreenCapturer::CreateMappedTexture(winrt::com_ptr<ID3D11Texture2D> src_texture,
...
std::pair<int, int> UwpScreenCapturer::resolution() const { return _dimensions; }
...
void UwpScreenCapturer::OnFrame(std::vector<uint8_t> bytes, int width, int height) {
    ...
    const int conversionResult = libyuv::ConvertToI420(
            plane_y, videoFrameLength, stride_y, plane_uv, stride_uv, buffer.get()->MutableDataY(),
            buffer.get()->StrideY(), buffer.get()->MutableDataU(), buffer.get()->StrideU(),
            buffer.get()->MutableDataV(), buffer.get()->StrideV(), 0, 0,  // No Cropping
            width, height, dst_width, dst_height, libyuv::kRotate0, libyuv::FOURCC_ARGB);
