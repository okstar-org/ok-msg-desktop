...
VideoSink::VideoSink(OkRTCHandler *handler, std::string peerId) : handler(handler), _peer_id(std::move(peerId)) { RTC_DCHECK(handler); }
  ...
  case webrtc::VideoFrameBuffer::Type::kI420: {
    ...
    if (frame.rotation() != webrtc::kVideoRotation_0) {
      webrtc::VideoFrame rotated_frame = webrtc::VideoFrame::Builder()
                                             .set_video_frame_buffer(webrtc::I420Buffer::Rotate(*v_buffer, frame.rotation()))
    ...
    break;
  default:
    RTC_LOG(LS_WARNING) << "Not supported frame type: " << webrtc::VideoFrameBufferTypeToString(v_buffer->type());
    break;
  }
