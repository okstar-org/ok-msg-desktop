...
void OpenAL::checkAlError() noexcept {
    ...
    if (al_err != AL_NO_ERROR) qWarning("OpenAL error: %d", al_err);
}
...
bool OpenAL::initOutput(const QString& deviceName) {
    ...
    // init master volume
    //    alListenerf(AL_GAIN, Nexus::getProfile()->getSettings()->getOutVolume() * 0.01f);
    checkAlError();
...
void OpenAL::playAudioBuffer(uint sourceId, const int16_t* data, int samples, unsigned channels,
    ...
    if (!(alOutDev && outputInitialized)) return;
