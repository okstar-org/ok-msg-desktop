 ...
 * This mean for clients with caps node starting with: psi+ and also for nodes starting with psi and
 * having word "fork" or "plus" somewhere inside, "psi-plus" icons will be used. For psi-ny (New
 * Year edition) icon caps whould start with "psi" and have "ny" somewhere in the middle.
 *
 ...
 *
 * Now for example we need to lookup icon for caps node "psiplus.com". The most still mathing item
 * here is "psi", (psi+ won't match because psiplus.com doesn't start with psi+). And we don't have
 * anything like "psip" or "psipl".. So we review just "psi" (all its items consequently) Both items
 * in "psi" have clarification list. For the first item we take its clarification list ["fork",
 * "plus"] and review if any item is in "psiplus.com". The "plus" will be found, so the icon
 * "psi-plus" will be returned.
 *
 * Note: It's quite regular for caps node to start with "https" but current client_icons.txt almost
 * doesn't have such records. It just means it heavily rely on detected client names instead of
 * caps. Client name from its side maybe taken from caps node when there is not other way to detect.
 * Example:
...
public:
    ...
    QString iconsetPath(QString name, Iconset::Format format = Iconset::Format::Psi) {
        if (format == Iconset::Format::Psi) {
            ...
            const auto& dataDirs = QStandardPaths::locateAll(QStandardPaths::GenericDataLocation,
    ...
    void loadIconset(Iconset* to, Iconset* from) {
        if (!to) {
            ...
            if (from) qWarning("from->name() = '%s'", qPrintable(from->name()));
            return;
        }
        if (!from) {
            qWarning("PsiIconset::loadIconset(): 'from' iconset is NULL!");
            if (to) qWarning("to->name() = '%s'", qPrintable(to->name()));
            return;
        }
    ...
    PsiIcon* jid2icon(const Jid& jid, const QString& iconName) {
        ...
        if (jid.node().isEmpty() || status_icons.useServicesIcons) {
            for (const StatusIconsets::IconsetItem& item : qAsConst(status_icons.list)) {
                if (item.regexp.isEmpty() ? jid.node().isEmpty()
                                          : (item.regexp.indexIn(jid.domain()) != -1)) {
    ...
    Iconset systemIconset(bool* ok) {
        ...
        *ok = def.load(":/iconsets/system/default");
        ...
        //        if (PsiOptions::instance()->getOption("options.iconsets.system").toString() !=
        //        "default") {
        //            Iconset is;
        //            is.load(iconsetPath("system/" +
        //            PsiOptions::instance()->getOption("options.iconsets.system").toString()));
        //
        //            loadIconset(&def, &is);
        //        }
    ...
    Iconset* defaultRosterIconset(bool* ok) {
        ...
        *ok = def->load(":/iconsets/roster/default");
        ...
        //        if (PsiOptions::instance()->getOption("options.iconsets.status").toString() !=
        //        "default") {
        //            Iconset is;
        //            is.load(iconsetPath("roster/" +
        //            PsiOptions::instance()->getOption("options.iconsets.status").toString()));
        //
        //            loadIconset(def, &is);
        //        }
    ...
    Iconset moodsIconset(bool* ok) {
        ...
        *ok = def.load(iconsetPath("moods/default"));
        ...
        //        if (PsiOptions::instance()->getOption("options.iconsets.moods").toString() !=
        //        "default") {
        //            Iconset is;
        //            is.load(iconsetPath("moods/" +
        //            PsiOptions::instance()->getOption("options.iconsets.moods").toString()));
        //
        //            loadIconset(&def, &is);
        //        }
    ...
    Iconset activityIconset(bool* ok) {
        ...
        *ok = def.load(iconsetPath("activities/default"));
        ...
        //        if (PsiOptions::instance()->getOption("options.iconsets.activities").toString() !=
        //        "default") {
        //            Iconset is;
    ...
    Iconset clientsIconset(bool* ok) {
        ...
        *ok = def.load(iconsetPath("clients/default"));
        ...
        if (PsiOptions::instance()->getOption("options.iconsets.clients").toString() != "default") {
            ...
            is.load(iconsetPath(
                    "clients/" +
                    PsiOptions::instance()->getOption("options.iconsets.clients").toString()));
    ...
    Iconset affiliationsIconset(bool* ok) {
        ...
        *ok = def.load(iconsetPath("affiliations/default"));
        ...
        if (PsiOptions::instance()->getOption("options.iconsets.affiliations").toString() !=
            "default") {
            Iconset is;
            is.load(iconsetPath(
                    "affiliations/" +
    ...
    QList<Iconset*> emoticons() {
        ...
        const auto names =
                PsiOptions::instance()->getOption("options.iconsets.emoticons").toStringList();
        for (const QString& name : names) {
            ...
            delete is;
            is = new Iconset;
            if (is->load(iconsetPath(name, Iconset::Format::KdeEmoticons),
                         Iconset::Format::KdeEmoticons)) {
                is->addToFactory();
                emo.append(is);
            } else
                delete is;
...
PsiIconset::PsiIconset() : QObject(QCoreApplication::instance()) {
    ...
    d->status_icons.useServicesIcons =
            PsiOptions::instance()
                    ->getOption("options.ui.contactlist.use-transport-icons")
                    .toBool();
...
bool PsiIconset::loadRoster() {
    ...
    for (const QVariant& service : services) {
        QString val = PsiOptions::instance()
                              ->getOption(PsiOptions::instance()->mapLookup(
                                                  "options.iconsets.service-status", service) +
                                          ".iconset")
                              .toString();
    ...
    QStringList customicons = PsiOptions::instance()->getChildOptionNames(
            "options.iconsets.custom-status", true, true);
    ...
    if (d->cur_clients != cur_clients) {
        ...
        auto readClientsDesc = [&](const QString& filePath) {
            ...
            /* file format: <icon res name> <left_part_of_cap1#inside1#inside2>,<left_part_of_cap2>
                next line the same.
            */
            if (!capsConv.open(QIODevice::ReadOnly)) return false;
            ...
            for (auto& checkList : cm) {
                std::sort(checkList.begin(), checkList.end(), [](const auto& a, const auto& b) {
        ...
        auto customPath =
                PsiOptions::instance()->getOption("options.iconsets.clients-capsfile").toString();
        if (customPath.isEmpty() || !readClientsDesc(customPath)) {
            QStringList dirs = ApplicationInfo::dataDirs();
            for (const auto& dataDir : dirs)
                if (readClientsDesc(dataDir + QLatin1String("/client_icons.txt"))) break;
        }
...
void PsiIconset::loadStatusIconDefinitions() {
    ...
    if (PsiOptions::instance()
                ->mapKeyList("options.iconsets.service-status")
                .contains("transport")) {
        PsiIconset::Private::StatusIconsets::IconsetItem item;
        item.iconset = PsiOptions::instance()
                               ->getOption(PsiOptions::instance()->mapLookup(
                                                   "options.iconsets.service-status", "transport") +
                                           ".iconset")
                               .toString();
        d->status_icons.list.append(item);
    }
    ...
    foreach (const QString& base,
             PsiOptions::instance()->getChildOptionNames(
                     "options.iconsets.custom-status", true, true)) {
        PsiIconset::Private::StatusIconsets::IconsetItem item;
...
bool PsiIconset::loadAll() {
    if (!loadSystem() || !loadRoster()) return false;
...
void PsiIconset::reloadRoster() {
    ...
    if (operator!=(d->cur_service_status, cur_service_status) || operator!=(d->cur_custom_status,
                                                                            cur_custom_status)) {
        QStringList rosterIconsets;

        QMap<QString, QString>::Iterator it = cur_service_status.begin();
        for (; it != cur_service_status.end(); ++it)
            if (!rosterIconsets.contains(it.value())) rosterIconsets << it.value();
...
PsiIcon* PsiIconset::transportStatusPtr(QString name, int s) {
    ...
    QVariantList serviceicons =
            PsiOptions::instance()->mapKeyList("options.iconsets.service-status");
    if (serviceicons.contains(name)) {
        const Iconset* is =
                roster.value(PsiOptions::instance()
                                     ->getOption(PsiOptions::instance()->mapLookup(
                                                         "options.iconsets.service-status", name) +
                                                 ".iconset")
                                     .toString());
        if (is) {
...
PsiIcon* PsiIconset::statusPtr(UserListItem* u) {
    ...
    if (!u->presenceError().isEmpty())
        ...
        s = STATUS_ASK;
    else if ((u->subscription().type() == Subscription::From ||
              u->subscription().type() == Subscription::None) &&
...
void PsiIconset::stripFirstAnimFrame(Iconset* is) {
    if (is) d->stripFirstAnimFrame(*is);
}
...
PsiIconset* PsiIconset::instance() {
    if (!instance_) instance_ = new PsiIconset();
    return instance_;
}
