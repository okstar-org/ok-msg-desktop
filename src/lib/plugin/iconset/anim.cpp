...
public:
    ...
    int numFrames() const { return frames.count(); }
 ...
 */
Anim::Anim() { d = new Private(); }

/**
 * Creates animation from QByteArray \a data.
 */
Anim::Anim(const QByteArray& data) { d = new Private(&data); }

/**
 * Creates shared copy of Anim \a anim.
 */
Anim::Anim(const Anim& anim) { d = anim.d; }
 ...
 */
const QPixmap& Anim::framePixmap() const { return d->frames[d->frame].impix.pixmap(); }

/**
 * Returns QImage of current frame.
 */
const QImage& Anim::frameImage() const { return d->frames[d->frame].impix.image(); }

/**
 * Returns Impix of current frame.
 */
const Impix& Anim::frameImpix() const { return d->frames[d->frame].impix; }

/**
 * Returns total number of frames in animation.
 */
int Anim::numFrames() const { return d->numFrames(); }

/**
 * Returns the number of current animation frame.
 */
int Anim::frameNumber() const { return d->frame; }

/**
 * Returns Impix of animation frame number \a n.
 */
const Impix& Anim::frame(int n) const { return d->frames[n].impix; }

/**
 * Returns \c true if numFrames() == 0 and \c false otherwise.
 */
bool Anim::isNull() const { return !numFrames(); }

/**
 * Returns \c true when animation is paused.
 */
bool Anim::paused() const { return d->paused; }

/**
 * Continues the animation playback.
 */
void Anim::unpause() {
    if (!isNull() && paused()) d->unpause();
}
 ...
 */
void Anim::setMainThread(QThread* thread) { animMainThread = thread; }

/**
 * Get the Anim's main thread.
 */
QThread* Anim::mainThread() { return animMainThread; }
