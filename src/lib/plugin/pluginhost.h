...
class PluginHost : public QObject,
  ...
  Q_OBJECT
  Q_INTERFACES(
      StanzaSendingHost IqFilteringHost ok::plugin::OptionAccessingHost
          ShortcutAccessingHost IconFactoryAccessingHost ActiveTabAccessingHost
              ApplicationInfoAccessingHost AccountInfoAccessingHost
                  PopupAccessingHost ContactStateAccessingHost
                      PsiAccountControllingHost EventCreatingHost
                          ContactInfoAccessingHost SoundAccessingHost
                              ok::plugin::EncryptionSupport PluginAccessingHost
...
public:
  ...
  void sendMessage(int account, const QString &to, const QString &body,
                   const QString &subject, const QString &type) override;
  QString uniqueId(int account) override;
  QString escape(const QString &str) override;

  // IqFilteringHost
  void addIqNamespaceFilter(const QString &ns,
                            IqNamespaceFilter *filter) override;
  void addIqNamespaceFilter(const QRegExp &ns,
                            IqNamespaceFilter *filter) override;
  void removeIqNamespaceFilter(const QString &ns,
                               IqNamespaceFilter *filter) override;
  void removeIqNamespaceFilter(const QRegExp &ns,
                               IqNamespaceFilter *filter) override;

  // OptionAccessingHost
  void setPluginOption(const QString &option, const QVariant &value) override;
  QVariant
  getPluginOption(const QString &option,
  ...
  bool hasCaps(int account, const QString &jid,
               const QStringList &caps) override;
  ...
  void initPopup(const QString &text, const QString &title, const QString &icon,
                 int type) override;
  void initPopupForJid(int account, const QString &jid, const QString &text,
  ...
  void addToolBarButton(QObject *parent, QWidget *toolbar, int account,
                        const QString &contact);
  bool hasToolBarButton();

  void addGCToolBarButton(QObject *parent, QWidget *toolbar, int account,
                          const QString &contact);
  ...
  bool appendingChatMessage(int account, const QString &contact, QString &body,
  ...
  void setStatus(int account, const QString &status,
                 const QString &statusMessage) override;
  bool appendSysMsg(int account, const QString &jid,
                    const QString &message) override;
  bool appendSysHtmlMsg(int account, const QString &jid,
                        const QString &message) override;
  void subscribeBeforeLogin(QObject *context,
                            std::function<void(int account)> callback) override;
  void subscribeLogout(QObject *context,
                       std::function<void(int account)> callback) override;
  ...
  void createNewEvent(int account, const QString &jid, const QString &descr,
                      QObject *receiver, const char *slot) override;
  ...
  QString installChatLogJSDataFilter(
      const QString &js,
      OkPlugin::Priority priority = OkPlugin::PriorityNormal) override;
