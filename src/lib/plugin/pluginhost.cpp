...
PluginHost::PluginHost(PluginManager *manager, const QString &pluginFile)
    : manager_(manager), plugin_(nullptr), file_(pluginFile),
      priority_(OkPlugin::PriorityNormal), loader_(nullptr), valid_(false),
      connected_(false), enabled_(false), hasInfo_(false),
      infoString_(QString()) {
  updateMetadata();
}
...
void PluginHost::updateMetadata() {
  ...
  description_ =
      md.value(QLatin1String("description") + curLangFull).toString();
...
bool PluginHost::load() {
  ...
  hasToolBarButton_ =
      qobject_cast<ToolbarIconAccessor *>(plugin_) ? true : false;
  hasGCToolBarButton_ =
      qobject_cast<GCToolbarIconAccessor *>(plugin_) ? true : false;
...
bool PluginHost::enable() {
  ...
  if (!enabled_ && load()) {
    if (!connected_) {
      ...
      ApplicationInfoAccessor *aia =
          qobject_cast<ApplicationInfoAccessor *>(plugin_);
      if (aia) {
      ...
      GCToolbarIconAccessor *gtia =
          qobject_cast<GCToolbarIconAccessor *>(plugin_);
      if (gtia) {
...
bool PluginHost::incomingXml(int account, const QDomElement &e) {
  ...
  // try iq filters
    ...
    bool (IqNamespaceFilter::*handler)(int account, const QDomElement &xml) =
        nullptr;
 ...
 */
void PluginHost::addIqNamespaceFilter(const QString &ns,
                                      IqNamespaceFilter *filter) {
 ...
 * \param filter Filter to be registered
 */
void PluginHost::addIqNamespaceFilter(const QRegExp &ns,
                                      IqNamespaceFilter *filter) {
  // #ifndef PLUGINS_NO_DEBUG
  //     qDebug("add nsx");
  // #endif
 ...
 */
void PluginHost::removeIqNamespaceFilter(const QString &ns,
                                         IqNamespaceFilter *filter) {
 ...
 * Note that \a filter object is never deleted by this function.
 */
void PluginHost::removeIqNamespaceFilter(const QRegExp &ns,
                                         IqNamespaceFilter *filter) {
...
void PluginHost::setPluginOption(const QString &option, const QVariant &value) {
  ...
  QString optionKey =
      QString("%1.%2.%3")
          .arg(PluginManager::pluginOptionPrefix, shortName(), option);
...
void PluginHost::connectShortcut(const QKeySequence &shortcut,
                                 QObject *receiver, const char *slot) {
  //    GlobalShortcutManager::instance()->connect(shortcut, receiver, slot);
}

void PluginHost::disconnectShortcut(const QKeySequence &shortcut,
                                    QObject *receiver, const char *slot) {
...
QString
PluginHost::appCapsVersion() { // this stuff is incompatible with new caps 1.5
...
QString
PluginHost::appProfilesDir(ApplicationInfoAccessingHost::HomedirType type) {
...
QString PluginHost::appCurrentProfileDir(
    ApplicationInfoAccessingHost::HomedirType type) {
...
QString PluginHost::getPgpKey(int account) {
  return manager_->getPgpKey(account);
}
...
void PluginHost::subscribeBeforeLogin(QObject *context,
                                      std::function<void(int)> callback) {
  connect(manager_, &PluginManager::accountBeforeLogin, context, callback);
}

void PluginHost::subscribeLogout(QObject *context,
                                 std::function<void(int account)> callback) {
...
void PluginHost::addToolBarButton(QObject *parent, QWidget *toolbar,
                                  int account, const QString &contact) {
...
void PluginHost::addGCToolBarButton(QObject *parent, QWidget *toolbar,
                                    int account, const QString &contact) {
...
void PluginHost::initPopup(const QString &text, const QString &title,
                           const QString &icon, int type) {
  manager_->initPopup(text, title, icon, type);
}

void PluginHost::initPopupForJid(int account, const QString &jid,
                                 const QString &text, const QString &title,
                                 const QString &icon, int type) {
  manager_->initPopupForJid(account, jid, text, title, icon, type);
}

int PluginHost::registerOption(const QString &name, int initValue,
                               const QString &path) {
...
void PluginHost::setupChatTab(QWidget *tab, int account,
...
bool PluginHost::hasCaps(int account, const QString &jid,
...
void PluginHost::setStatus(int account, const QString &status,
                           const QString &statusMessage) {
  manager_->setStatus(account, status, statusMessage);
}

bool PluginHost::appendSysMsg(int account, const QString &jid,
                              const QString &message) {
  return manager_->appendSysMsg(account, jid, message);
}

bool PluginHost::appendSysHtmlMsg(int account, const QString &jid,
                                  const QString &message) {
  return manager_->appendSysHtmlMsg(account, jid, message);
}

void PluginHost::createNewEvent(int account, const QString &jid,
                                const QString &descr, QObject *receiver,
                                const char *slot) {
  manager_->createNewEvent(account, jid, descr, receiver, slot);
}

void PluginHost::createNewMessageEvent(int account,
...
QString PluginHost::installChatLogJSDataFilter(const QString &js,
                                               OkPlugin::Priority priority) {
