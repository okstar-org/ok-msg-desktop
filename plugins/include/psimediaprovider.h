...
public:
  virtual ~Plugin() {}
  virtual Provider *
  createProvider(const QVariantMap &param = QVariantMap()) = 0;
};
...
public:
  ...
  inline PPayloadInfo()
      : id(-1), clockrate(-1), channels(-1), ptime(-1), maxptime(-1) {}
};
...
public:
  ...
  virtual void lookup(int types, QObject *receiver,
                      std::function<void(const PFeatures &)> &&callback) = 0;
  virtual void monitor(int types, QObject *receiver,
                       std::function<void(const PFeatures &)> &&callback) = 0;
};
...
public:
  ...
  virtual void
  dumpPipeline(std::function<void(const QStringList &)> callback) = 0;

  HINT_SIGNALS
      : HINT_METHOD(started()) HINT_METHOD(preferencesUpdated())
            HINT_METHOD(audioOutputIntensityChanged(int intensity))
                HINT_METHOD(audioInputIntensityChanged(int intensity))
                    HINT_METHOD(stoppedRecording()) HINT_METHOD(stopped())
...
public:
  ...
  HINT_SIGNALS : HINT_METHOD(started()) HINT_METHOD(preferencesUpdated())
                     HINT_METHOD(stopped()) HINT_METHOD(paused())
                         HINT_METHOD(error())
};
}; // namespace PsiMedia
} // namespace plugin
...
Q_DECLARE_INTERFACE(ok::plugin::PsiMedia::Plugin, "org.okstar.msg.psimedia.Plugin/1.5")
Q_DECLARE_INTERFACE(ok::plugin::PsiMedia::Provider, "org.okstar.msg.psimedia.Provider/1.5")
Q_DECLARE_INTERFACE(ok::plugin::PsiMedia::FeaturesContext,
                    "org.okstar.msg.psimedia.FeaturesContext/1.4")
Q_DECLARE_INTERFACE(ok::plugin::PsiMedia::RtpChannelContext,
                    "org.okstar.msg.psimedia.RtpChannelContext/1.5")
Q_DECLARE_INTERFACE(ok::plugin::PsiMedia::RtpSessionContext,
                    "org.okstar.msg.psimedia.RtpSessionContext/1.4")
Q_DECLARE_INTERFACE(ok::plugin::PsiMedia::AudioRecorderContext,
                    "org.okstar.msg.psimedia.AudioRecorderContext/1.4")
